...
function withLargeShirt(currentRunner){
    return currentRunner.shirt_size == "L";
}

largeShirts = runners.filter(withLargeShirt);
...
function ticketDonation(element){
    ticketPriceTotal.push(element.donation);
}

runners.forEach(ticketDonation);
...
cost = ticketPriceTotal.reduce(getSum);
console.log(cost);
